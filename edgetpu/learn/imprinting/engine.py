...
.. note::
  ...
  +   Most importantly, the input model has new architecture requirements. For details,
      read `Retrain an image classification model on-device
      <https://coral.withgoogle.com/docs/edgetpu/retrain-classification-ondevice/>`_.
  ...
  +   :func:`~ImprintingEngine.Train` now requires a second argument for the class ID you want to
      ...
      class ID.)
  +   :func:`~ImprintingEngine.TrainAll` requires a different format for the input data. It now uses
  ...
  +   New methods :func:`~ImprintingEngine.ClassifyWithResizedImage` and
      :func:`~ImprintingEngine.ClassifyWithInputTensor` allow you to immediately perform inferences,
      though you can still choose to save the trained model as a ``.tflite`` file with
      :func:`~ImprintingEngine.SaveModel`.
"""
...
class ImprintingEngine(object):
  ...
  def SaveModel(self, output_path):
    ...
    :func:`~ImprintingEngine.ClassifyWithResizedImage` or
    :func:`~ImprintingEngine.ClassifyWithInputTensor`.
