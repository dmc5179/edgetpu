...
class DetectionCandidate(object):
  """A data structure that represents one detection candidate (label id, score, and bounding box).
     ...
     This is returned by methods :func:`~DetectionEngine.DetectWithImage` and
     :func:`~DetectionEngine.DetectWithInputTensor`."""
  ...
  def __init__(self, label_id, score, x1, y1, x2, y2):
    ...
    #: pass to the :func:`~DetectionEngine.DetectWithImage` or
    #: :func:`~DetectionEngine.DetectWithInputTensor` method. [0, 0] is always the top-left corner.
  ...
  def DetectWithImage(self, img, threshold=0.1, top_k=3,
    ...
    Args:
      ...
      keep_aspect_ratio (bool): If True, keep the image aspect ratio the same when down-sampling
        ...
        :func:`~edgetpu.utils.image_processing.ResamplingWithOriginalRatio()` function).
