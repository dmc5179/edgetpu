...
def _GetShape(model):
  ...
  Returns:
    (width, height)
  """
  basic_engine = BasicEngine(test_utils.TestDataPath('imprinting', model))
  _, height, width, _ = basic_engine.get_input_tensor_shape()
  return (width, height)


def _BenchmarkForTraining(model, data_set):
  ...
  Returns:
    float, training time.
  """
  shape = _GetShape(model)
  engine = ImprintingEngine(test_utils.TestDataPath('imprinting', model), keep_classes=False)
...
if __name__ == '__main__':
  ...
  models, reference = test_utils.ReadReference(
