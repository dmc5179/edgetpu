...
class EdgeTpuLearnUtilsTest(unittest.TestCase):
  ...
  def testAppendFullyConnectedAndSoftmaxLayerToModel(self):
    in_model_name = (
        'mobilenet_v1_1.0_224_quant_embedding_extractor_edgetpu.tflite')
    in_model_path = test_utils.TestDataPath(in_model_name)
    ...
    _, embedding_vector = in_engine.RunInference(input_tensor)
    ...
    try:
      ...
      _, result = out_engine.RunInference(input_tensor)
  ...
  def testAppendFullyConnectedAndSoftmaxLayerToModelWithInvalidInputPath(self):
    in_model_name = ('invalide_file_path.tflite')
    in_model_path = test_utils.TestDataPath(in_model_name)
