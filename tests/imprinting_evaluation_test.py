...
class ImprintingEngineEvaluationTest(unittest.TestCase):
  ...
  def _GetOutputNumberClasses(self, model_path):
  ...
  def _TransferLearnAndEvaluate(self, model_path, keep_classes, dataset_path,
    ...
    imprinting_engine.TrainAll(train_input)
    print('----------------     Training finished   -----------------')
    output_model_path = tempfile.NamedTemporaryFile(suffix='.tflite')
    imprinting_engine.SaveModel(output_model_path.name)
  ...
  def _TestOxford17FlowersSingle(self, model_path, keep_classes, expected):
    top_k_range = len(expected)
    ret = self._TransferLearnAndEvaluate(
        test_utils.TestDataPath(model_path),
        keep_classes,
  ...
  def testOxford17FlowersL2NormModelNotKeepClasses(self):
    self._TestOxford17FlowersSingle(
  ...
  def testOxford17FlowersL2NormModelKeepClasses(self):
    self._TestOxford17FlowersSingle(
        'imprinting/mobilenet_v1_1.0_224_l2norm_quant.tflite',
        keep_classes=True,
