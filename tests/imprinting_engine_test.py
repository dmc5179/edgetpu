...
class TestImprintingEnginePythonAPI(unittest.TestCase):
  ...
  def _GetImageShape(self, model_path):
  ...
  def _ClassifyImageByInference(self, engine, image_shape, data_dir, image_name, label_id, score):
    with open(os.path.join(data_dir, image_name), 'rb') as f:
      with Image.open(f) as img:
        img = img.resize((image_shape[0], image_shape[1]), Image.NEAREST)
        ret = engine.ClassifyWithResizedImage(img, top_k=1)
        self.assertEqual(len(ret), 1)
        self.assertEqual(ret[0][0], label_id)
        self.assertGreater(ret[0][1], score)

  def _TrainAndTest(self, model_path, output_model_path, training_datapoints,
                    test_datapoints, keep_classes=False):
    engine = ImprintingEngine(model_path, keep_classes)
    image_shape = self._GetImageShape(model_path)
    data_dir = test_utils.TestDataPath('imprinting')
    # Train.
    for training_datapoint in training_datapoints:
      engine.Train(test_utils.PrepareImages(training_datapoint['image_names'],
    ...
    for test_datapoint in test_datapoints:
      self._ClassifyImage(engine, data_dir, test_datapoint['image_name'],
                          test_datapoint['label_id'], test_datapoint['score'])

  def _TrainAndTestRunInference(self, model_path, training_datapoints,
                    test_datapoints, keep_classes=False):
    engine = ImprintingEngine(model_path, keep_classes)
    image_shape = self._GetImageShape(model_path)
    data_dir = test_utils.TestDataPath('imprinting')
    ...
    for test_datapoint in test_datapoints:
      self._ClassifyImageByInference(engine, image_shape, data_dir, test_datapoint['image_name'],
                          test_datapoint['label_id'], test_datapoint['score'])

  # Test full model using RunInference, not keeping base model classes.
  def testRunInferenceTrainingL2NormModelNotKeepClasses(self):
    training_datapoints = [
    ...
    for model_path in self._MODEL_LIST:
      with self.subTest():
        self._TrainAndTestRunInference(model_path, training_datapoints,
                                       test_datapoints, keep_classes=False)

  # Test full model, not keeping base model classes.
  def testTrainingL2NormModelNotKeepClasses(self):
    training_datapoints = [
        {'image_names': ['cat_train_0.bmp'], 'label_id': 0},
        {'image_names': ['dog_train_0.bmp'], 'label_id': 1},
        {'image_names': ['hotdog_train_0.bmp', 'hotdog_train_1.bmp'], 'label_id': 2}
    ...
    for model_path in self._MODEL_LIST:
      with self.subTest():
        with tempfile.NamedTemporaryFile(suffix='.tflite') as output_model_path:
          self._TrainAndTest(model_path, output_model_path.name, training_datapoints,
                             test_datapoints, keep_classes=False)

  # Test full model, keeping base model classes.
  def testTrainingL2NormModelKeepClasses(self):
    training_datapoints = [
    ...
    for model_path in self._MODEL_LIST:
      with self.subTest():
        with tempfile.NamedTemporaryFile(suffix='.tflite') as cat_only_model_path:
          self._TrainAndTest(model_path, cat_only_model_path.name,
                             training_datapoints, [], keep_classes=False)
          # Retrain based on cat only model.
          with tempfile.NamedTemporaryFile(suffix='.tflite') as output_model_path:
            self._TrainAndTest(cat_only_model_path.name,
                               output_model_path.name,
                               retrain_training_datapoints,
                               test_datapoints,
                               keep_classes=True)
    ...
    for model_path in self._MODEL_LIST:
      with self.subTest():
        with tempfile.NamedTemporaryFile(suffix='.tflite') as output_model_path:
          ...
          train_input = [(
              test_utils.PrepareImages(image_list, data_dir, image_shape)
